...
class DatabaseManager:
    ...
    def dataBetweenId(self, table, cols, minId, maxId=False, convert=True):
        ...
        query = (f"select r.id, r.tai, {cols} from reply_raw r "
                 f"join {table} d on r.id = d.raw_id "
                 f"where r.id between {minId} and {maxId}")
    ...
    def pollDbConf(self, date, ignoreActors=None):
        ...
        for table in self.allTables(ignoreActors):
            try:
                _ = self.closestId(table, minId, maxId)
